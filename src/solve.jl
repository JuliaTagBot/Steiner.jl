...
function steiner_system()
    @polyvar x[1:2] a[1:5] c[1:6] y[1:2, 1:5] P[1:6]
    #tangential conics
    f = a[1]*x[1]^2 + a[2]*x[1]*x[2] + a[3]*x[2]^2 + a[4]*x[1]  + a[5]*x[2] + 1;
    ∇ = differentiate(f, x)
    #5 conics
    g = c[1]*x[1]^2 + c[2]*x[1]*x[2] + c[3]*x[2]^2 + c[4]*x[1]  + c[5]*x[2] + 1;
...
const startconics_startsolutions = begin
    startconics = SVector{25}(readdlm(joinpath(@__DIR__, "..", "deps", "startconics.txt"), '\t', ComplexF64))
...
function assemble_tracker()
    ...
    pathtracker(F, startsolutions, parameters=parameters, p₁=startconics, p₀=(@SVector randn(25)))
end
...
function solve_conics(M::Matrix)
    tangential_conics = Vector{Float64}[]#SVector{5, Float64}[]
    solve_conics(conic -> push!(tangential_conics, conic), M)
    tangential_conics
end

function solve_conics(output!::F, M::Matrix) where {F<:Function}
    p₁, startsolutions = startconics_startsolutions
    p₀ = SVector{25}(complex.(M))
...
function handle_solve_conics(conics_input)
    ...
    @time tangential_conics = solve_conics(M)
    nreal = length(tangential_conics)
    Dict("tangential_conics" => tangential_conics, "nreal" => nreal)
end
